...
###        joblib.dump(scaler, os.path.join(model_folder_path, "scaler_classification16.pkl"))
###        
...
###    
###    class_weight_nn = {0: 9.0, 1: 150, 2: 250, 3: 130}
###
###    # Build the model
...
###    
###    model.save(os.path.join(model_folder_path, 'ensemble_modelFINP16.h5'))
###
...
###        scaler = joblib.load(os.path.join(model_folder_path, "scaler_classification16.pkl"))
...
###        model = load_model(os.path.join(model_folder_path, 'ensemble_modelFINP16.h5'))
...
###            return (
###                #df[['Predicted Breakdown']], Count: {num_unique_non_zero_codes},Codes: {', '.join(unique_non_zero_codes)}
###                #f"Breakdown of {', '.join(unique_non_zero_codes)} might occur."
###                #f"Breakdown code predicted as: Count: {num_unique_non_zero_codes}, Codes: {','.join(unique_non_zero_codes)} ."
###                f"Breakdown of {', '.join(unique_non_zero_codes)} might occur."
###                
###            )
###        else:
